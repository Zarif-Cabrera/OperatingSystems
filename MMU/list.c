...
void list_coalese_nodes(list_t *l){ 
  ...
  while (current != NULL) {
  // Calculate the size of the merged block if prev and current are adjacent
    int mergedSize = current->blk->end - prev->blk->start + 1;

    // Use compareSize to check adjacency
    if (prev->blk->end + 1 == current->blk->start && 
      compareSize(mergedSize, prev->blk)) {
